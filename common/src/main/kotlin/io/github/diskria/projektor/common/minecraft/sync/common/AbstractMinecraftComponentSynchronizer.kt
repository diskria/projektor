    ...
    fun getLatestComponent(project: Project, minecraftVersion: MinecraftVersion): MinecraftComponent {
        ...
        val components = cache?.takeIf { nowMillis() - it.lastSyncMillis < cacheDurationMillis } ?: runBlocking {
            val versions = fetchComponents()
                ...
                .filterKeys { loader?.getSupportedVersionRange()?.includesVersion(it) ?: true }
                .mapValues {
        ...
        return components.versions
            ...
            ?: gradleError("$componentName latest version not found for Minecraft ${minecraftVersion.asString()}")
    }

    fun getLatestVersion(project: Project, minecraftVersion: MinecraftVersion): String =
