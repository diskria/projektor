...
open class ProjektExtension @Inject constructor(objects: ObjectFactory) : ProjectExtension() {
    ...
    fun minecraftMod(block: MinecraftMod.() -> Unit = {}): MinecraftMod = script {
        ...
        tasks.named<Jar>("jar") {
            manifest {
                ...
                val specificationVendor by mod.owner.toAutoNamedProperty(`Train-Case`)

                val implementationVersion by jarVersion.toAutoNamedProperty(`Train-Case`)
                val implementationTitle by mod.displayName.toAutoNamedProperty(`Train-Case`)
                val implementationVendor by mod.owner.toAutoNamedProperty(`Train-Case`)
    ...
    private fun applyCommonConfiguration(
        projekt: IProjekt,
        jarBaseName: String = projekt.name,
        ...
        tasks.named<Jar>("jar") {
            from("LICENSE") {
                rename { oldName ->
                    oldName + Constants.Char.UNDERSCORE + projekt.name
                }
            }
        ...
        publishingTarget.orNull?.publish(this, projekt)
    }
                ...
                ModSide.entries.forEach { side ->
                    ...
                    named(sideName) {
                        ...
                        if (hasSide) {
                            ...
                            programArgs("--username", "${mod.owner}-$sideName")
